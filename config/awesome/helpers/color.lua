...
-- Returns a value that is clampped to interval edges if it falls outside the interval
...
function _color.rgb_to_hex(color)
	...
	local a = clamp(color.a or color[4] or 255, 0, 255)
...
function _color.hex_to_hsv(color)
	color = _color.hex_to_rgb(color)
	local C_max = math.max(color.r, color.g, color.b)
	local C_min = math.min(color.r, color.g, color.b)
...
function _color.rand_hex(lb_angle, ub_angle)
	return C.color({
		h = math.random(lb_angle or 0, ub_angle or 360),
		s = 70,
		v = 90,
	}).hex
end
...
function _color.relative_lighten(color, amount)
	color = C.color({ hex = color })
	color.l = color.l + (1 - color.l) * amount
	return color.hex
end

function _color.relative_darken(color, amount)
	color = C.color({ hex = color })
	color.l = color.l - color.l * amount
	return color.hex
end

-- Pywal like functions
function _color.pywal_blend(color1, color2)
	color1 = C.color({ hex = color1 })
	color2 = C.color({ hex = color2 })
